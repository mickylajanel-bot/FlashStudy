...
export function DeckEditor({ deck, onSave, onCancel }: DeckEditorProps) {
  ...
  const handleSave = () => {
    ...
    if (validCards.length === 0) {
      alert("Please ensure your cards have questions, answers, and at least one correct choice selected for Multiple Choice types.");
      return;
    }
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 rounded-lg bg-bg-sidebar border border-border-subtle hover:border-accent-indigo text-gray-400 hover:text-accent-indigo disabled:opacity-20 transition-all"
            title="Undo"
          >
      ...
      <div className="space-y-10">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
                    ...
                    <div className="md:col-span-3 space-y-3">
                      ...
                      <div className="grid grid-cols-1 gap-2">
                        {CARD_TYPES.map((t) => (
                          <button
                            key={t.type}
                            onClick={() => updateCard(index, { type: t.type })}
                                  ...
                                  <button
                                    ...
                                    className={`px-4 py-4 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all border ${
                                      isCorrectOption
                                        ...
                                        : 'bg-gray-50 text-gray-400 border-border-subtle hover:bg-emerald-50 hover:text-emerald-500 hover:border-emerald-200 disabled:opacity-30'
                                    }`}
            ...
            <button
              onClick={addCard}
              className="w-full border-2 border-dashed border-border-subtle hover:border-accent-indigo/50 rounded-3xl py-8 flex flex-col items-center justify-center gap-3 text-gray-400 hover:text-accent-indigo transition-all group bg-bg-card/50"
            >
        ...
        {/* Footer Actions */}
        <div className="pt-12 flex justify-end gap-6 border-t border-border-subtle">
          <button
            onClick={onCancel}
            className="px-8 py-3 text-gray-500 font-bold text-xs tracking-widest uppercase hover:text-gray-900 transition-all"
          >
            Go Back
          </button>
          <button
            ...
            className="px-12 py-3 bg-accent-indigo text-white font-bold text-xs tracking-widest uppercase rounded-xl shadow-xl shadow-accent-indigo/20 hover:bg-accent-indigo-dark transition-all active:scale-[0.98] disabled:opacity-20 flex items-center gap-3"
          >
